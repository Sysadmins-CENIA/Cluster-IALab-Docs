...
* **Causa:** Estás leyendo tu dataset directamente desde la red (`/home` , `$ARCHIVE` o `$SCRATCH` ), saturando la conexión del clúster.
...
* **Solución:** No uses `apt` ni solicites instalaciones globales. Encapsula tus dependencias en tu entorno virtual (`Conda`/`venv`) o utiliza un contenedor (solicitando primero la habilitación de contenedores para tí).
