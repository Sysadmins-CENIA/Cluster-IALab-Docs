...
- Cada usuario es responsable de sus datos y código. Se recomienda realizar copias de seguridad de la información crítica en un sistema externo.
...
- Los usuarios del cluster IALAB tienen acceso a ciertos tipos de almacenamiento con cuotas máximas de uso:
  - **Home**: almacenamiento pequeño y personal. Pensado para guardar código y ambientes virtuales. Cuota de 50 GB por usuario.
  - **Workspaces**: almacenamiento de alta velocidad temporal y particular a cada nodo. Los datos de entrenamiento leídos por un experimento deben ser almacenados en este tipo de almacenamiento.Cuota de 200 GB máximo por usuario.
  - **Archive**: Almacenamiento a largo plazo para resultados importantes como Resultados finales, datos de publicaciones, backups importantes
                 Accesible a través del directorio **archive** en el home. Cuota de 200 GB máximo por usuario.
  - **Scratch**:Almacenamiento temporal para resultados intermedios como Salidas de jobs que necesitan almacenamiento a corto plazo, se debe limpiar regularmente. Accesible a través del directorio **scratch** en el home. Cuota de 500 GB máximo por usuario.
...
- La información contenida en los directorios home, scratch y archive no se borrará. Sin embargo, si por alguna razón un usuario está ocupando más espacio que el permitido por su cuota (por ejemplo, tras haber pedido una cuota mayor y luego ésta fue reducida), se le enviará un correo indicando que debe borrar información para estar bajo la cuota o debe volver a solicitar una renovación de cuota hasta una fecha especifica con la justificación de su supervisor. Si tras 14 días la violación de cuota persiste, se dará aviso al usuario y a su Supervisor de que en 7 días se borrará toda la información del usuario del directorio especifico.
